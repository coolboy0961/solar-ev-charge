...
- **自動再接続の継続**: セッション喪失または未接続を検知している限り、`loop` は再接続を繰り返し試行する
  - 再接続失敗で切断状態にラッチせず、再起動なしで復帰できる
  - 連続試行の間隔は `RECONNECT_BACKOFF`（既定30秒）で間引く
...
| `RECONNECT_BACKOFF` | 30000 (30秒) | Wi-SUN 再接続試行の最小間隔 |
