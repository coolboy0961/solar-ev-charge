...
```
surplus         = solar_surplus_power_avg (90秒移動平均)            (正=売電=余剰)
...
30秒ごと (Main Control Loop):
  ...
  [充電中] choose 分岐 (上から順に評価、最初に一致した分岐のみ実行):
    ...
    1. REDUCE: solarpower < (current_amps × 200V + 400W) AND amps > 5A
       → target A に変更 + stabilize 60秒 + reduce cooldown 90秒
    2. LOW SOLAR: solarpower < 800W
       → disable delay timer 開始 (idle の場合のみ)
    3. DEFAULT (十分な発電):
       → disable delay timer キャンセル (動作中の場合)
       → IF target > current AND reduce cooldown idle → set target + stabilize 60秒
       → IF current - target >= 3A → set target + stabilize 60秒 (デッドバンド)
...
別オートメーション:
  [enable delay 満了]  → surplus_avg > 1200W 再チェック
  ...
  [充電停止ガード] charger OFF 検出 → guard timer (5分) ※原因問わず
  [充電完了] charging_state = Complete → timer 全キャンセル + 通知
  [手動OFF] Solar Charging OFF → 充電停止 + 全 timer cancel
  [ケーブル接続初期化] charge_flap off→on 検出 + Solar Charging ON
                         → amps を 5A にプリセット
                         (Tesla 自走時に過剰電流で余剰を消失させない)
  [ケーブル接続ガード] charger ON 検出 + Solar Charging ON + guard idle + 余剰<1200W
...
| 充電開始再チェック (surplus_avg) | 1200W | automations.yaml (enable delay expired) |
...
| Stabilize | 60秒 (電流変更後) / 1分 (開始後) | automations.yaml |
...
| Entity ID | 説明 | 計算ロジック |
|-----------|------|-------------|
| `sensor.solar_surplus_power` | 余剰電力 W (瞬時値) | `smart_meter_power × -1` (正=余剰) |
| `sensor.solar_surplus_power_avg` | 余剰電力 W (90秒移動平均) | メインループ・充電開始判定で使用 |
